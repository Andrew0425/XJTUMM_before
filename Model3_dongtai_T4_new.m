...
while 1
    ...
    sorted_vec = sort(Dens_fore, 'descend');
    Max_1_index = find(Dens_fore == sorted_vec(1), 1);
    Max_100_index = find(Dens_fore == sorted_vec(round(length(Loc_current)/3)), 1);
    Max_200_index = find(Dens_fore == sorted_vec(2*round(length(Loc_current)/3)), 1);
    % [~, Max_index] = max(Dens_fore);
    Loc_1_Max = Loc_current(Max_1_index, :)';
    Loc_100_Max = Loc_current(Max_100_index, :)';
    Loc_200_Max = Loc_current(Max_200_index, :)';
    %AUV
    if t_fore > t0
        Dist = sqrt(sum((Loc_search1-Loc_1_Max).^2));
        Direct = (Loc_1_Max - Loc_search1)/Dist;
        if Dist > verticle1*dt
            Loc_search1 = Loc_search1 + Direct*dt;
        else
            Loc_search1 = Loc_1_Max;
        end
    ...
    %Sub1
    if t_fore > t1
        Dist = sqrt(sum((Loc_search2-Loc_100_Max).^2));
        Direct = (Loc_100_Max - Loc_search2)/Dist;
        if Dist > verticle2*dt
            Loc_search2 = Loc_search2 + Direct*dt;
        else
            Loc_search2 = Loc_100_Max;
        end
    ...
    %Sub2
    if t_fore > t1
        Dist = sqrt(sum((Loc_search3-Loc_200_Max).^2));
        Direct = (Loc_200_Max - Loc_search3)/Dist;
        if Dist > verticle3*dt
            Loc_search3 = Loc_search3 + Direct*dt;
        else
            Loc_search3 = Loc_200_Max;
        end
...
view(-68,35)
title('Path of AUV And Other Submersible','FontName','Times New Roman','FontSize',16)
legend('Path of AUV','Path of Submersible 1','Path of Submersible 2','FontName','Times New Roman','FontSize',12)
%%
