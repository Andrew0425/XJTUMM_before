...
Sheet2 = xlsread('C:\Users\86189\Desktop\2023\007(t3mox)\002(t3mox)\001_sheet2.xlsx');	%读取反射光线的数据OOOOOOOOO
Sheet3 = xlsread('C:\Users\86189\Desktop\2023\007(t3mox)\002(t3mox)\001_sheet3.xlsx');	%读取法向量的数据OOOOOOOOOO
Sheet4 = xlsread('C:\Users\86189\Desktop\2023\007(t3mox)\002(t3mox)\001.xlsx');
X=Sheet2(2:NumberMirror+1,2:2);%定日镜横坐标OOOOOOOOOOOO
Y=Sheet2(2:NumberMirror+1,3:3);%定日镜纵坐标OOOOOOOOOOO
Z=Sheet4(1:NumberMirror,3:3);
Wm=Sheet4(1:NumberMirror,4:4);
Hm=Sheet4(1:NumberMirror,5:5);
...
Nearest=Sheet2(2:NumberMirror+1,8:22);%离目标定日镜最近的5个
...
for D=1:3:178
    ...
    NLSUM=0;
    %目标定日镜中心点
    M_i = [X(mi,1),Y(mi,1),Z(mi,1)];%OOOOOOOOOOOOOOOOO
    %目标定日镜法向量
    ...
    %潜在遮挡定日镜中心点
        for mk=1:5
            M_j = [Nearest(mi,3*mk-2),Nearest(mi,3*mk-1),Nearest(mi,3*mk)];%OOOOOOOOOOOOOOOOO
            R_j = ([0,-200,80]-M_j)/norm([0,-200,80]-M_j);%假设吸收塔中心位置确定
            %潜在遮挡定日镜法向量
...
for w=-Wm(mi,1)/2:0.6:Wm(mi,1)/2
    for h=-Hm(mi,1)/2:0.6:Hm(mi,1)/2
H_i = [w,h,0];
x_i = H_i(1);
y_i = H_i(2);
...
x2=(c21*xi1-a21*zi1)/c21;y2=(c21*yi1-b21*zi1)/c21;
        if x1<=Wm(mi,1)/2 && x1>=-Wm(mi,1)/2 && y1<=Hm(mi,1)/2 && y1>=-Hm(mi,1)/2%OOOOOOOOOOOOOOOOO
            N=N+1;
        elseif x2<=Wm(mi,1)/2 && x2>=-Wm(mi,1)/2 && y2<=Hm(mi,1)/2 && y2>=-Hm(mi,1)/2%OOOOOOOOOOOOOOOOO
            N=N+1;
        end
    end
end
    ...
    for h=-Hm/2:1:Hm/2
        ...
        NSB(mi,(D+2)/3)=1-0.6*0.6*N/(Wm(mi,1)*Hm(mi,1));
        N=0.92*(1-0.6*0.6*N/(Wm(mi,1)*Hm(mi,1)))*(0.99321-0.0001176*d+1.97*(10^(-8))*d^2)*yita_cos*NL/NLSUM;
        NN(mi,(D+2)/3)=N;
        gonglv=gonglv+Wm(mi,1)*Hm(mi,1)*N*DNI(1,(D+2)/3);

        
    end
GONGLV((D+2)/3,1)=gonglv;
end
